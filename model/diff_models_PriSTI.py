from model.layers_PriSTI import *
    ...
    def __init__(self, config, inputdim=1, target_dim=36, is_itp=False):
        ...
        if self.is_itp:
            ...
            self.itp_modeling = GuidanceConstruct(channels=self.itp_channels, nheads=config["nheads"], target_dim=target_dim,
                                            order=2, include_self=True, device=config["device"], is_adp=True,
                                            adj_file=config["adj_file"], proj_t=16)
        ...
        if config["adj_file"] == 'PM25':
            # self.adj = get_similarity_PM25(thr=0.1)
            self.adj = create_time_correlation_matrix_pandas_PM25()
            print(self.adj.shape)
        elif config["adj_file"] == 'ETTh1':
            self.adj = get_similarity_ETTh1(thr=0.1)
        elif config["adj_file"] == 'Satellite':
            self.adj = get_similarity_Satellite(thr=0.1)
        elif config["adj_file"] == 'sun':
            self.adj = get_similarity_sun(thr=0.1)
