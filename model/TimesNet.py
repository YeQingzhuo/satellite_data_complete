...
class Model(nn.Module):
    ...
    def imputation(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask,is_history=False):
        device = x_enc.device    
        noise = torch.randn_like(x_enc) * self.history_prob
        mask = mask.to(device)
        x_enc = x_enc + noise * mask
        ...
        dec_out = dec_out + \
                  (means[:, 0, :].unsqueeze(1).repeat(
                      1, self.pred_len + self.seq_len, 1))
        dec_out_np = dec_out.detach().cpu().numpy() if hasattr(dec_out, 'cpu') else np.array(dec_out)
        std_dev = np.std(dec_out_np)
  
        sigma = std_dev * 0.5    
        dec_out_denoised = gaussian_filter(dec_out_np, sigma=sigma)

          
        dec_out_denoised = torch.tensor(dec_out_denoised).to(dec_out.device) if hasattr(dec_out, 'device') else dec_out_denoised
        return dec_out
