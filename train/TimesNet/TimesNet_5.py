...
def train_TimesNet( ): 
    ...
    parser.add_argument('--WX', type=int, default=1, help='weight of X')
    parser.add_argument('--WN', type=int, default=1, help='weight of neighbour')
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            data_preprocess_full_pth = os.path.join(args.root_path, args.data_full_path)
            ...
            for epoch in range(args.train_epochs):
                ...
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,mask2,data2) in enumerate(train_loader1):
                    ...
                    if loss.numel() > 0:
                        running_loss_N1 += torch.sum(loss)/loss.numel()
                ...
                sum_loss_N1=0
                sum_loss_N1 = running_loss_N1/len(train_loader1)
                  

                 

                print('>>>>>>>start neighbor training>>>>>>>')
                ...
                mask_data = mask * current_data      
                 
                m2 = scio.loadmat(data_preprocess_full_pth)
                matrix_past_fill=t.Tensor(m2['data']).to(device)     
                matrix_past_fill = matrix_past_fill.cpu().numpy()    
                 
                start_row = 0
                 
                end_row = 50
                size_input = matrix_past_fill
                size_input = size_input[:, :, start_row:end_row]
                 
                time_sample_matrix_fill =window_slide(size_input,end_row,window_size=args.window_size,stride=args.stride)    
                ...
                time_sample_matrix=window_slide(matrix_past,end_row,window_size=args.window_size,stride=args.stride)     
                time_sample_matrix = time_sample_matrix.astype(np.float32)   
                 
                batch_size = current_data_3D.size(0)
                dim1 = current_data_3D.size(1)
                dim2 = current_data_3D.size(2)
                mask_data = mask_data.view(batch_size,dim1,dim2)     
                mask_data =  mask_data.cpu().numpy()     
                
                topk_idxs,neighbor_matrix,weight_neighbor = find_nearest_neighbors(mask_data,time_sample_matrix,time_sample_matrix_fill,neighbor_num=args.neighbor_num)   
                 
                begin_row = 0  
                if args.data_name == 'Satellite_part_3D'or args.data_name == 'sun_3D':
                    begin_row = topk_idxs*60*args.stride     
                else:
                    begin_row = topk_idxs*24*args.stride     
                ...
                neighbor_data, neighbor_loader = data_provider(args,flag='neighbor',epoch=epoch,begin_row=begin_row,mask=None)
                 
                running_loss_N2 = 0

                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,mask2,data3) in enumerate(neighbor_loader):
                    model_optim.zero_grad()
                    batch_x = batch_x.float().to(device)
                    batch_x_mark = batch_x_mark.float().to(device)
                    ...
                    loss = custom_loss(outputs[mask3 == 1], batch_x[mask3 == 1])
                    if loss.numel() > 0:
                            running_loss_N2 += torch.sum(loss)/loss.numel()
                    loss=torch.sum(loss)/loss.numel()
                    loss = loss*args.WN
                ...
                sum_loss_N2=0
                sum_loss_N2 = running_loss_N2/len(neighbor_loader)


                sum_loss_N = sum_loss_N1+sum_loss_N2
                with open(args.result_path,'a+') as f:
                    print('sum_loss_N1:',sum_loss_N1,file=f) 
                    print('sum_loss_N2:',sum_loss_N2,file=f) 
                    print('sum_loss:',sum_loss_N,file=f) 
