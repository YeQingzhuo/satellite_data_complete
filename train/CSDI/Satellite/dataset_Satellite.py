...
class Dataset(Dataset):
    def __init__(self, current_day,num,eval_length=60, section_num=8, use_index_list=None, missing_ratio=0.1, seed=0,
                 use_hist_cond=False,
        ...
        np.random.seed(
            seed)   
        self.observed_values = np.zeros((2781,60,11))
        self.observed_masks = []
        self.origin_values = []
        self.origin_masks = []
        self.matched_values = []
        ...
        path = ("./data/Satellite/Satellite_2781.pk")
         
        if os.path.isfile(path) == False:
            observed_origin = pd.read_csv("../matrix_filling_pq/data/Satellite/Satellite.csv", index_col="date", parse_dates=True).values[:, :]
             
            for i in range(2781):   
                self.observed_values[i] = observed_origin[i * 60:(i + 1) * 60, :]
            ...
            tmp_values = self.observed_values.reshape(-1, 11)
            tmp_masks = self.observed_masks.reshape(-1, 11)
            mean = np.zeros(11)
            std = np.zeros(11)
            for k in range(11):
...
class neighbor_Dataset(Dataset):
    def __init__(self,eval_length=60, section_num=8, use_index_list=None, missing_ratio=0.1, seed=0,
                 use_hist_cond=False,
