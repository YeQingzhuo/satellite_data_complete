...
from model.CSDI import CSDI_Satellite
from dataset_Satellite import get_dataloader,get_dataloader_add_history
...
foldername = "./save_CSDI/Satellite_nfold" + str(args.nfold) + "_X1+X2_" + current_time + "/"
...
def train_CSDI_add_history(lr,num_epoches,current_day,num,nsample,WX,window_size,stride,data_name,result_pth,data_file_path,Normal,data_preprocess_full_pth,data_preprocess_full_name):
    model = CSDI_Satellite(config, args.device).to(args.device)   
