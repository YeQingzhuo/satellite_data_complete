...
class Dataset(Dataset):
    def __init__(self, current_day,num,eval_length=24, section_num=8, use_index_list=None, missing_ratio=0.1, seed=0,
        ...
        np.random.seed(
            seed)   
        self.observed_values = np.zeros((350,24,36))
        self.observed_masks = []
        self.origin_values = []
        self.origin_masks = []
        self.matched_values = []
        ...
        if os.path.isfile(path) == False:
            observed_origin = pd.read_csv("../matrix_filling_pq/data/PM25/pm25_ground.txt", index_col="datetime", parse_dates=True).values[23:, :]
            ...
            tmp_values = self.observed_values.reshape(-1, 36)
            tmp_masks = self.observed_masks.reshape(-1, 36)
            mean = np.zeros(36)
            std = np.zeros(36)
            for k in range(36):
