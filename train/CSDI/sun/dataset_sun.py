...
class Dataset(Dataset):
    def __init__(self, current_day,num,eval_length=60, section_num=8, use_index_list=None, missing_ratio=0.1, seed=0,
        ...
        np.random.seed(
            seed)   
        self.observed_values = np.zeros((281,60,12))
        self.observed_masks = []
        self.origin_values = []
        self.origin_masks = []
        self.matched_values = []
        ...
        path = ("./data/sun/sun_281.pk")
         
        if os.path.isfile(path) == False:
            observed_origin = pd.read_csv("../matrix_filling_pq/data/sun/sun.csv", index_col="date", parse_dates=True).values
