...
def train_NTM_with_Predata(lr,WX,batch_size,sample_rate,embedding_dim,epoches,nc,count,num_day,window_size,stride,data_name,data_pth,result_pth,save_dir_pth): 
    num_epoches = epoches
    ...
    model=NTM(data.shape[0],data.shape[1],data.shape[2],embedding_dim)
    print(model)
    ...
    for epoch in range(num_epoches):
        ...
        neighbor_matrix,weight_neighbor = find_nearest_neighbors(data_neigh,time_sample_matrix,time_sample_matrix_fill)
        ...
        for n in range(neighbor_matrix.shape[0]):
            ...
            for k in range(data_neigh_tensor.shape[2]):
                ...
                for i,data_i in enumerate(train_neigh_loader,0):   
                    ...
                    outputs = model(o_inputs, d_inputs, t_inputs,is_history=True).to(device)
        ...
        if epoch>=0:
            ...
            test_er,test_mae,test_rmse,test_mape=get_score(dataset=test_eval,model=model,count=3000,sp=data.shape)
            train_er,train_mae,train_rmse,train_mape=get_score(dataset=train_eval,model=model,count=3000,sp=data.shape)

             
            with open(result_pth,'a+') as f:
                print('!!!!X2 evaluate result!!!!',file=f)
...
window_size = 2
stride = 1
data_name = 'Satellite_part_3D'
data_pth='../matrix_filling_pq/data/Satellite/Satellite_part_3D.mat'
result_pth='../matrix_filling_pq/result/NTM/Satellite/result_2.txt'
save_dir_pth = '../matrix_filling_pq/save_model/NTM/Satellite/2/'


train_NTM_with_Predata(lr,WX,batch_size,sample_rate,embedding_dim,epoches,nc,count,num_day,window_size,stride,data_name,data_pth,result_pth,save_dir_pth)
print("over")
