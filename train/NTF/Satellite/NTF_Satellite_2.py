...
def train_NTF_with_Predata(lr,WX,batch_size,sample_rate,embedding_dim,epoches,nc,count,num_day,window_size,stride,data_name,data_pth,result_pth,save_dir_pth): 
    num_epoches = epoches
    ...
    for epoch in range(num_epoches):
        ...
        for n in range(neighbor_matrix.shape[0]):
            ...
            for k in range(data_neigh_tensor.shape[2]):
                ...
                for i,data_i in enumerate(train_neigh_loader,0):   
                    ...
                    t_inputs_period = idx2seq(d_inputs.cpu().numpy(), period=48)
                    outputs = model(o_inputs, d_inputs, t_inputs,t_inputs_period,is_history=True).to(device)
        ...
        if epoch>=0:
            ...
            test_er,test_mae,test_rmse,test_mape=get_score_NTF(dataset=test_eval,model=model,count=3000,sp=data.shape)
            train_er,train_mae,train_rmse,train_mape=get_score_NTF(dataset=train_eval,model=model,count=3000,sp=data.shape)
...
result_pth='../matrix_filling_pq/result/NTF/Satellite/result_2.txt'
save_dir_pth = '../matrix_filling_pq/save_model/NTF/Satellite/2/'


train_NTF_with_Predata(lr,WX,batch_size,sample_rate,embedding_dim,epoches,nc,count,num_day,window_size,stride,data_name,data_pth,result_pth,save_dir_pth)
print("over")
